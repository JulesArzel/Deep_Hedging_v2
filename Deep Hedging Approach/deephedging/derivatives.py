...
class NegativeCallSpread(BaseDerivative):
    """
    Buyer of a call spread: holds +call(K1) − call(K2).
    payoff_fn returns the negated spread (what the 'hedger' must deliver),
    consistent with the NegativeEuropeanOption convention in pfhedge.
    hedger.price() will be negative; use -hedger.price() for the buyer premium.
    """
    def __init__(self, underlier, strike_long, strike_short, maturity=1.0):
        super().__init__()
        self.underlier = underlier
    ...
    def payoff_fn(self):
        spot = self.underlier.spot
        return (torch.clamp(spot[..., -1] - self.strike_short, min=0)
                - torch.clamp(spot[..., -1] - self.strike_long, min=0))
    ...
    def extra_repr(self):
        return f"NegativeCallSpread(strike_long={self.strike_long}, strike_short={self.strike_short}, maturity={self.maturity})"
