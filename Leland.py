...
class Leland():
    ...
    def __init__(self, S, K, T, sigma, r, costs, position, frequence, q=0, option_type='Call'):
        self.S = S
        self.K = K
        self.T = T
        self.vol = sigma 
        ...
        self.frequence = frequence.lower()
        if self.frequence == 'minute':
            self.steps = int(self.T * 252 * 6.5 * 60)
        elif self.frequence == 'hourly':
            self.steps = int(self.T * 252 * 6)
        elif self.frequence == 'daily':
            self.steps = int(self.T * 252)
        elif self.frequence == 'weekly':
            self.steps = int(self.T * 50)
        else: 
            raise ValueError("frequence must be 'minute', 'hourly', 'daily' or 'weekly'")  
        dt = self.T / self.steps
        self.vol = self.vol*np.sqrt(1+(np.sqrt(2/np.pi)*self.costs/(self.vol*dt)))
    
    def pricing_Leland(self, St, t):
        St = np.asarray(St)
        t = np.asarray(t)
    ...
    def Hedging(self):
        ...
        S = self.price_trajectory()
        dt = self.T / self.steps 

        Delta = np.zeros(self.steps + 1)
        Buy = np.zeros(self.steps + 1)
        Price = np.zeros(self.steps + 1)
        Cash = np.zeros(self.steps + 1)
        Holdings = np.zeros(self.steps + 1)
        Portfolio = np.zeros(self.steps + 1)
        Hedg_Error = np.zeros(self.steps + 1)

        Delta[0] = sign * self.Delta(self.S, 0)
        Buy[0] = Delta[0]
        Price[0] = self.pricing_Leland(self.S, 0)
        if Buy[0]>=0 :
        ...
        Price[1:] = self.pricing_Leland(S[1:], times[1:])
